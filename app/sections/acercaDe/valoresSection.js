...
function ValueCard({ title, icon, text, bgColor, headerColor }) {
  return (
    <div className={`p-8 sm:p-12 rounded-lg text-white w-full sm:w-80 relative ${bgColor} h-auto`}>
      <div
        className={`text-center py-2 font-bold text-lg rounded-t-lg absolute -top-8 left-1/2 transform -translate-x-1/2 w-3/4 ${headerColor}`}
      >
        {title}
      </div>
      <div className="flex justify-center my-6 text-4xl sm:text-5xl">{icon}</div>
      <hr className="border-t-2 border-white my-4" />
      <p className="text-center mt-4">{text}</p> {/* Centrado aquí */}
    </div>
  );
}
  ...
  return (
    <div id="values" className="px-8 py-12 md:px-16 lg:px-32 text-center">
      <h2
        ...
        className="text-5xl font-bold text-[#014421] text-center mb-20 font-serif border-l-8 border-[#5DBE66] pl-4 mx-auto inline-block"
      >
