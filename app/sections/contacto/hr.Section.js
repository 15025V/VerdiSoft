    ...
    <div id='schedule' className="px-6 py-16 sm:px-12 md:px-24 lg:px-36 xl:px-48 bg-gradient-to-br to-[#e1e1e1] flex flex-col md:flex-row items-center justify-center text-center md:text-left gap-8 w-full"> 
      ...
      <div className="bg-white p-8 sm:p-10 md:p-14 rounded-[20px] sm:rounded-[25px] md:rounded-[30px] shadow-2xl max-w-4xl border-t-8 border-[#5DBE66] w-full md:w-1/2"> 
        <div className="flex flex-col space-y-4 text-left">
          {[
            { day: 'lunes', label: translations.lunes?.title},
            { day: 'sabado', label: translations.sabado?.title },
            { day: 'domi', label: translations.domi?.title}
          ].map(({ day, label, time }) => (
            <div key={day} className="text-base sm:text-lg text-gray-900 font-serif flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              <p className="text-gray-800 text-justify">{time}</p>
            </div>
          ))}
        </div>
      </div>
