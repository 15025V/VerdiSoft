          ...
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product, index) => (
              <div key={index} className="p-6 text-center bg-white hover:shadow-xl transition-all border border-black"> {/* Contorno negro */}
                ...
                <div className="mt-4">
                  <a href={`/about/${product.name.toLowerCase().replace(/\s+/g, '-')}`} className="inline-flex items-center text-green-800 hover:text-green-700 transition-colors"> {/* Enlace con flecha */}
                    Ver producto
